...
int main(int argc, char **argv){
  ...
  for (ii = 0; ii < nfiles; ii++) {
   ...
   /* get the center pixel */
    ...
    assert(hgetr8(header, "CRPIX1", &center_x));
    assert(hgetr8(header, "CRPIX2", &center_y));
...
#elif defined (EUVIBUILD || CORBUILD || defined AIABUILD)
  ...
  if ( strcmp(BpBcode,"PB") == 0 )
	...
	/* Keep only data within certain radius range  */
        ...
        if ((rho[i][jj] * dist * ARCSECRAD / RSUN > INSTR_RMAX) ||
            (rho[i][jj] * dist * ARCSECRAD / RSUN < INSTR_RMIN)) {
          pBval[i][jj] = -999.0;
	  ...
	  if ( abs(pBval[i][jj] + 999) > QEPS)  /* check for -999 values (missing blocks) */
	    pBval[i][jj] =
	      *(pbvector + imsize * jj + i) * 1.e10 * 0.79;
#if defined MARSEILLES
  	      *(pbvector + imsize * jj + i) *         0.79; /* Marseilles scaling */ 
#endif

#elif (defined EITBUILD || defined EUVIBUILD || defined AIABUILD)
	  if ( abs(pBval[i][jj] + 999) > QEPS)  /* check for -999 values (missing blocks) */
	    pBval[i][jj] = 
	      (float) *(pbvector + imsize * jj + i);	
