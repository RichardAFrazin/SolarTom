 ...
 * Oct. 29, 2004
 *
 * Dump CRPIX1, CRPIX2, SOLAR_R, and then image data to a file as an
 * unformatted string.
 *
 */
...
int main(int argc, char **argv) {
  ...
  int i, j;
  int size = SIZE;
  double center_x, center_y, solar_r, exp_time;
  ...
  assert(hgetr8(fits_head, "SOLAR_R", &solar_r));
  assert(hgetr8(fits_head, "EXPTIME", &exp_time));
  ...
  assert(fwrite(&solar_r, sizeof(double), 1, output_file) == 1);
