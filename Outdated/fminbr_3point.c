 ...
 */
/*
 *    see fminbr.c for a version that requires NO guesses.
 *
 *     see vfminbr.c for an example driver code.  
 *
 */
...
#define SQRT_EPSILON 1.e-21

double fminbr_3point(a,b,xi0,xi1,xi2,f,tol)
double a;				/* Left border | of the range	*/
double b;  				/* Right border| the min is seeked*/
double xi0, xi1, xi2;  /* initial 3 points to start parabola, preferably ordered best to worst */ 
double (*f)(double x);			/* Function under investigation	*/
double tol;				/* Acceptable tolerance		*/
{
  double fxi0, fxi1, fxi2;
  ...
  /* put f(x), f(v) , f(w) in asceding order */

  fxi2 = f(xi2); fxi1 = f(xi1); fxi0 = f(xi0);
  
  if ( (fxi2 > fxi1) && (fxi2 > fxi0)){
    w = xi2; fw = fxi2;
    if (fxi1 > fxi0){
      v = xi1; x = xi0; 
      fv = fxi1; fx = fxi0;
  ...
  fprintf(stdout,"fminbr_3point: starting values:\n x = %g, fx = %g,\n v = %g, fv = %g,\n w = %g, fw = %g\n",x,fx,v,fv,w,fw);
  fflush(stdout);

 /* First step - always gold section -- not anymore!!!!*/ 
  /*
  v = a + r*(b-a);  fv = (*f)(v);      
  x = v;  w = v;
  fx=fv;  fw=fv;
